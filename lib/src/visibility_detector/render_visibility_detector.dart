...
import 'package:better_player_plus/src/visibility_detector/visibility_detector.dart';
import 'package:better_player_plus/src/visibility_detector/visibility_detector_controller.dart';
...
mixin RenderVisibilityDetectorBase on RenderObject {
  ...
  void _scheduleUpdate([ContainerLayer? layer]) {
    ...
    final bool isFirstUpdate = _updates.isEmpty;
    _updates[key] = () {
      if (bounds == null) {
    ...
    if (updateInterval == Duration.zero) {
      ...
      assert(_timer!.isActive, '_timer must be active');
    }
  }

  VisibilityInfo _determineVisibility(ContainerLayer? layer, Rect bounds) {
    if (_disposed || layer == null || !layer.attached || !attached) {
      // layer is detached and thus invisible.
      return VisibilityInfo(key: key, size: _lastVisibility[key]?.size ?? Size.zero);
    }
    ...
    for (int index = ancestors.length - 1; index > 0; index -= 1) {
      ...
      final Rect? parentClip = parent.describeClipBounds();
      if (parentClip != null) {
    ...
    if (_lastPaintClipBounds != null) {
      clip = clip.intersect(MatrixUtils.transformRect(transform, _lastPaintClipBounds!));
    }
  ...
  void paint(PaintingContext context, Offset offset) {
    if (onVisibilityChanged != null) {
      ...
      _lastPaintTransform = Matrix4.fromFloat64List(context.canvas.getTransform())
        ..translateByDouble(offset.dx, offset.dy, 0, 1);

      _compositionCallbackCanceller?.call();
      _compositionCallbackCanceller = context.addCompositionCallback((Layer layer) {
        assert(!debugDisposed!, 'Layer must not be disposed');
